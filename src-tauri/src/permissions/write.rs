...
#[tauri::command]
pub fn set_write_permission(path: &str) -> Result<String, String> {
	...
	if let Some(permissions) = json["permissions"].as_array_mut() {
		if !permissions.iter().any(|p| {
			p["identifier"] == new_write_permission["identifier"]
				&& p["allow"] == new_write_permission["allow"]
		}) {
			permissions.push(new_write_permission);
		...
		json["permissions"] = json!([new_write_permission]);
