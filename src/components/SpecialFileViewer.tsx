import { FaExclamationTriangle, FaEye, FaDownload } from "react-icons/fa";
...
export default function SpecialFileViewer({ fileName }: SpecialFileViewerProps) {
	...
	const getFutureFeatures = (filename: string): string[] => {
		...
		const features: { [key: string]: string[] } = {
			'.pdf': ['Vista previa de PDF', 'Búsqueda de texto', 'Navegación por páginas'],
			...
			<div className="text-center max-w-md p-8">
				...
				<div className="bg-gray-800 rounded-lg p-4 mb-6 text-left">
					...
					<div className="flex items-center mb-2">
						...
						<span className="ml-2 text-gray-200">{getFileCategory(fileName)}</span>
					</div>
				</div>

				{/* Mensaje principal */}
				<p className="text-gray-300 mb-6 leading-relaxed">
					Este tipo de archivo requiere un visor especializado que será implementado
				...
				<div className="flex gap-4 justify-center">
					<button type="button" className="flex items-center px-4 py-2 bg-gray-700 text-gray-400 rounded-lg cursor-not-allowed" disabled>
						<FaEye className="mr-2" />
						Vista previa
					</button>
					<button type="button" className="flex items-center px-4 py-2 bg-gray-700 text-gray-400 rounded-lg cursor-not-allowed" disabled>
				...
				<p className="text-xs text-gray-500 mt-6">
					Mientras tanto, puedes usar aplicaciones externas para ver este archivo.
				</p>
			</div>
		</div>
	);
