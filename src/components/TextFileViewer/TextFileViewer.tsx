...
const analyzeAndGetIndentations = (data: string[][]) => {
  ...
  for (const row of data) {
    ...
    if (finMatch) {
      ...
      if (
        taskStack.length > 0 &&
        taskStack[taskStack.length - 1] === taskName
      ) {
        taskStack.pop();
      }
    }
...
function TextFileViewer({ path, delimiter }: TextFileViewerProps) {
  ...
  const notifications = useMemo(
    () =>
      unclosedTasks.map((task, index) => ({
        id: `${task.split("\\").at(-1)}-${index}`,
        message: `Tarea sin cerrar: ${task.split("\\").at(-1)}`,
      })),
    [unclosedTasks]
  );
  ...
  const displayedNotifications = notifications.filter(
    (n) => !dismissedNotifications.includes(n)
  );
  ...
  const handleEntryClick = (lineNumber: number) => {
    const row = tableRef.current?.querySelector(`tr:nth-child(${lineNumber})`);
    if (row) {
  ...
  return (
    <div className="w-full h-full" style={{ position: "relative" }}>
      ...
      {displayedNotifications.length > 0 && (
        <div className="absolute top-10 left-4 w-fit max-h-[40%] overflow-y-auto bg-[rgba(40,40,40,0.95)] text-blue-400 rounded-lg p-4 shadow-lg z-[2000]">
          <button
            type="button"
            onClick={() => setIsBannerExpanded(!isBannerExpanded)}
            className="mt-0 mb-3 border-b border-[#555] pb-2 cursor-pointer select-none bg-transparent text-white w-full text-left opacity-80 hover:opacity-100"
