...
function TextFileViewer({ path, delimiter }: TextFileViewerProps) {
  ...
  const [_isLogAnalysisExpanded, _setIsLogAnalysisExpanded] = useState(true);
  ...
  const debouncedReload = useCallback(() => {
    ...
    timeoutRef.current = setTimeout(async () => {
      try {
        ...
        const linesComplete = rows.map((row) => `${row[2]}`);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (data.length > 0) {
          setLogData(data);
          const linesComplete = data.map((row) => `${row[2]}`);
                    ...
                    <span
                      ...
                      onKeyUp={(e: React.KeyboardEvent<HTMLSpanElement>) => {
                        if (e.key === "Enter" || e.key === " ") {
                          handleEntryClick(notif.lineNumber);
                        }
                      }}
                      role="button"
                      tabIndex={0}
